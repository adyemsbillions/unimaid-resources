...
export default function TopicsPage() {
  const { subject_id } = useLocalSearchParams<{ subject_id: string }>()
  ...
  const fetchTopics = useCallback(async (pull = false) => {
    ...
    try {
      ...
      if (json.success && Array.isArray(json.data)) {
        ...
        Alert.alert("Error", json.error || "No topics")
      }
    } catch (e) {
          ...
          <View style={styles.list}>
            {topics.map((topic) => (
              <TouchableOpacity
                ...
                onPress={() => {
                  Alert.alert("Next", `Open lessons for Topic: ${topic.name} (ID: ${topic.id})`)
